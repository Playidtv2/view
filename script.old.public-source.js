...
function process() {
    var url = 'https://raw.githubusercontent.com/Playidtv2/dooballsod/main/config.json';
    $.ajax({
        url: url,
        async: false,
        dataType: 'json',
        ...
        error: function (err) {
            alert("เกิดข้อผิดพลาดขณะเตรียมใช้งานทีวี!\nโปรดลองอีกครั้งในภายหลัง!\n" + err);
        }
    });
}
...
$(function () {
    ...
    if (!domain.includes("tekmonts")) {
        ...
        if (answer) {
            ...
            alert("คราวหน้ากรุณาเข้าไปดูที่หน้าเดิม/!");
        }
    }
	process();
